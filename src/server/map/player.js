...
class Cell {
    ...
    move(playerX, playerY, playerTarget, slowBase, initMassLog, speedMultiplier = 1) {
        var target = {
        ...
        var boost = Math.max(1, speedMultiplier || 1);
        var deltaY = this.speed * Math.sin(deg) / slowDown * boost;
        var deltaX = this.speed * Math.cos(deg) / slowDown * boost;
...
exports.Player = class {
    ...
    init(position, defaultPlayerMass) {
        ...
        this.warpCooldownUntil = this.warpCooldownUntil || 0;
        this.warpBoostUntil = 0;
        this.beaconFragments = {};
    ...
    changeCellMass(cellIndex, massDifference) {
        if (!this.cells[cellIndex] || !massDifference) return 0;

        // Starfall event safety: score can grow forever, but playable mass cannot.
        // Otherwise a 35-minute match turns one ship's shield into a purple moon.
        const maxTotal = cfg.maxPlayerMassTotal || Infinity;
        if (massDifference > 0 && this.massTotal >= maxTotal) {
            return 0;
        }
        if (massDifference > 0 && this.massTotal + massDifference > maxTotal) {
            massDifference = maxTotal - this.massTotal;
        }

        const currentMass = this.cells[cellIndex].mass;
        if (currentMass + massDifference < 1) {
            massDifference = 1 - currentMass;
        }
    ...
    move(slowBase, gameWidth, gameHeight, initMassLog, speedMultiplier = 1) {
        ...
        for (let i = 0; i < this.cells.length; i++) {
            ...
            cell.move(this.x, this.y, this.target, slowBase, initMassLog, speedMultiplier);
...
exports.PlayerManager = class {
    ...
    getTopPlayers() {
        this.data.sort(function (a, b) {
            return (b.score || b.massTotal) - (a.score || a.massTotal);
        });
        var topPlayers = [];
        for (var i = 0; i < Math.min(10, this.data.length); i++) {
            topPlayers.push({
                ...
                mass: Math.round(this.data[i].massTotal),
                score: Math.round(this.data[i].score || this.data[i].massTotal)
            });
        }
        return topPlayers;
    }
